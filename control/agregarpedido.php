...
if (isset($_POST['agregar'])) {
    ...
    if ($fila_trabajador = mysqli_fetch_array($resultado_trabajador)) {
        ...
        if ($fila_cliente = mysqli_fetch_array($resultado_cliente)) {
            ...
            // Insertar pedido
            $consulta_insertar_pedido = "INSERT INTO Pedidos (id_cliente, productos, descripcion_pedido, fecha_realizacion, fecha_entrega, estatus, id_trabajador, costo_total)
                                        VALUES (?, ?, ?, ?, ?, 'Pendiente', ?, ?)";
            $stmt_insertar_pedido = mysqli_prepare($conexion, $consulta_insertar_pedido);
            mysqli_stmt_bind_param($stmt_insertar_pedido, "issisd", $id_cliente, $productos, $descripcion, $fechaRegisto, $fechaEntrega, $id_trabajador, $costo_total);
